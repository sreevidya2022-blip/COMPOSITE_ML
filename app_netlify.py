...
# Cache for models (persistent across function calls on Netlify)
_payload_cache = {}

def get_payload():
    """Get or load models from cache"""
    if not _payload_cache:
        loaded = load_models()
        if loaded:
            _payload_cache['data'] = loaded
        return loaded
    return _payload_cache.get('data')
...
def status():
    payload = get_payload()
    return jsonify({"models_loaded": payload is not None})
...
def train():
    ...
    try:
        payload = train_models(theory_path, fea_path, exp_path)
        _payload_cache['data'] = payload  # Cache the trained models
        return jsonify({
            "success": True,
            "r2_scores": payload["all_r2"],
            "exp_count": len(payload["exp_data"]),
        })
...
def predict_route():
    ...
    results = predict(bn, ao, payload)
    curves  = get_pd_curves(bn, ao, payload)
...
def inverse_predict_route():
    ...
    try:
        result = inverse_predict(targets, payload, weights=weights, grid_size=150)
